...
function initImport(){
  ...
  el.importJsonFile.addEventListener('change', (event)=>{
    ...
    showNotification('Importazione','Sovrascrivere dati attuali?',true,()=>{
      ...
      reader.onload = (e)=>{
        try{
          const importedRaw = JSON.parse(String(e.target.result||''));
          const imported = Array.isArray(importedRaw)
            ? importedRaw
            : (importedRaw && Array.isArray(importedRaw.products) ? importedRaw.products : null);
...
function main(){
  console.log('[IO Sano] main() start');
  refreshUI();
  checkUI();
  S.initState();
  initColumnDialog();
